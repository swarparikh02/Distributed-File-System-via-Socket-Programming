...
char NAME_TR[] = "pdf.tar"; // Defines the name of the tar archive file for file bundling
#define SIZE_BUFFER 1024 // Sets a constant for the buffer size used in data transfers
#define S_PORT 4222 // Specifies the port number (4222) for the server to listen on
char DIRT_BASEP[] = "/home/swar/ASP/S2"; // Sets the base directory for storing and retrieving files
...
{ // Main function to run the server
    ...
    if (s_fdt < 0) { // Checks if socket creation failed
    ...
    printf("Server-2 is ready it accepts only pdf files...\n"); // Informs that the server is ready for PDF operations
    while (1) { // Enters an infinite loop to handle client connections
        ...
        switch (typ_cmd) { // Switches based on the command type
            ...
            { // Handles the "retrieve" command
                ...
                if (nbyts < 0) { // Checks if reading failed
                    printf("Failure in reading...\n"); // Prints an error message
                }
                close(ffd); // Closes the file
                break; // Exits the case
            }
            case DELETE_COMMD: { // Handles the "delete" command
                char *file_r = strtok(NULL, ""); // Extracts the file path from the command
                if (!file_r) { // Checks if the file path is missing
                    close(fd_file); // Closes the client socket
                    break; // Exits the case
