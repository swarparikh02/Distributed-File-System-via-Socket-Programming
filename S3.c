...
char NAME_TR[] = "text.tar"; // Defines the name of the tar archive file for file bundling
#define SIZE_BUFFER 1024 // Sets a constant for the buffer size used in data transfers
#define S_PORT 4223 // Specifies the port number (4223) for the server to listen on
char DIRT_BASEP[] = "/home/parikh37/S3"; // Sets the base directory for storing and retrieving files
...
{ // Main function to run the server
    ...
    printf("Server-3 is ready it accepts only text files...\n"); // Informs that the server is ready for txt file operations
    while (1) { // Enters an infinite loop to handle client connections
        int fd_file = accept(s_fdt, NULL, NULL); // Accepts a new client connection
        if (fd_file < 0)
        { // Checks if accepting the connection failed
        ...
        switch (typ_cmd)
        { // Switches based on the command type
            case STORE_COMMD:
            { // Handles the "store" command
                ...
                if (ffd < 0) { // Checks if file opening failed
                    ...
                    while (lt_byts > 0)
                    { // Loops until all bytes are received
                ...
                { // Loops until all bytes are written
                    ...
                    if (w_byts != nbyts) { // Checks if writing failed
            ...
            { // Handles the "retrieve" command
                ...
                if (sz_file < 0) { // Checks if the file size is invalid
                    close(fd_file); // Closes the client socket
                    break; // Exits the case
                }
            ...
            { // Handles the "delete" command
                ...
                char t_fpath[512]; // Buffer to store the full file path
                snprintf(t_fpath, sizeof(t_fpath), "%s%s", DIRT_BASEP, file_r + 3); // Constructs the full path
                if (remove(t_fpath) != 0) { // Attempts to delete the file
                    printf("Failure in removing file..."); // Prints an error if deletion fails
                }
                break; // Exits the case
            }
            case GETTAR_COMMD: { // Handles the "maketar" command
                char t_fp[512]; // Buffer to store the tar file path
                snprintf(t_fp, sizeof(t_fp), "/tmp/%s", NAME_TR); // Constructs the path for the tar file
                char s_cmd[1024]; // Buffer for the tar command
                snprintf(s_cmd, sizeof(s_cmd), "cd %s && find . -type f -print | tar -cf %s -T -", DIRT_BASEP, t_fp); // Builds the command to create a tar archive
                system(s_cmd); // Executes the tar command
                int t_fd = open(t_fp, O_RDONLY); // Opens the tar file for reading
                long sz_file = -1; // Initializes tar file size to invalid
                if (t_fd >= 0) { // If the tar file was opened successfully
                    sz_file = lseek(t_fd, 0, SEEK_END); // Gets the tar file size
                    lseek(t_fd, 0, SEEK_SET); // Resets the file pointer to the start
                } else { // If opening the tar file failed
                    printf("Failure in opening tar..."); // Prints an error message
                }
                send(fd_file, &sz_file, sizeof(long), 0); // Sends the tar file size to the client
                if (sz_file < 0) { // Checks if the file size is invalid
                    close(fd_file); // Closes the client socket
                    remove(t_fp); // Deletes the temporary tar file
                    break; // Exits the case
                }
                char buf_r[SIZE_BUFFER]; // Buffer for reading tar file data
                ssize_t nbyts; // Tracks the number of bytes read
                while ((nbyts = read(t_fd, buf_r, SIZE_BUFFER)) > 0)
                { // Reads the tar file in chunks
                    send(fd_file, buf_r, nbyts, 0); // Sends the chunk to the client
                }
                if (nbyts < 0)
                { // Checks if reading failed
                    printf("Failure in reading tar..."); // Prints an error message
                }
                close(t_fd); // Closes the tar file
                remove(t_fp); // Deletes the temporary tar file
                break; // Exits the case
            }
            case PRINTFN_COMMD: { // Handles the "list" command
                ...
                if (!d) { // Checks if opening the directory failed
                ...
                { // Reads each directory entry
                    if (dt->d_type != DT_REG) continue; // Skips non-regular files
                    lst_f[ct] = strdup(dt->d_name); // Copies the file name into the array
                    ct++; // Increments the file counter
                    if (ct >= 256) { // Limits the number of files to 256
                        break; // Exits the loop
                    }
                }
